...
app.get('/auth/google/callback',
    ...
    function (req, res) {
        if (!req.user) return res.redirect('http://localhost:5173/account-create/sign-in');

        const token = jwt.sign({ user: req.user }, process.env.JWT_SECRET, { expiresIn: '1h' })
        const token1 = jwt.sign({ id: req.user._id }, process.env.JWT_SECRET, { expiresIn: "1h" })

        res.cookie("access_token", token1, { httpOnly: true, sameSite: true, secure: false })
        console.log(token1)

        res.redirect(`https://frontend-ce0qfkkdd-chandrashekharsainis-projects.vercel.app/?token=${encodeURIComponent(JSON.stringify(token))}`)
    });
...
app.listen(PORT, () => {
    ...
    mongoose.connect('mongodb://127.0.0.1:27017/videoCom').then(() => {
        console.log("Connected to MongoDB")
    })
